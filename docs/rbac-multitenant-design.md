...
> - **P8b DONE**: institute nav is permission-filtered (`href→permission` map + `useSession`); roles see only their menu items (access was already enforced).
> - **API feature-gating DONE**: `requireFeature()` added to 21 flag-area routes (lms/gamification/online-exams/library/banking/payroll/quality/transfers/equivalence). **Proven**: disabling `gamification.enabled` → `/api/student/gamification` returns **404** directly (not just the page). Fixed an over-broad map (`/institute/exams` no longer tied to the `exams.online` flag).
> - **✅ PROJECT COMPLETE (P1–P10 + polish).** All on the local `sinai_test` DB — **production untouched**. Committed to branch `feat/rbac-multitenant-platform` (not pushed). Remaining only if desired: push+PR, production-deploy runbook.
