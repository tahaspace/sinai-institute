...
**Graduation board (`graduates-result-sheet`) special layout:** the **final year** (الفرقة النهائية) shows the full per-course component detail; **each prior year** is a group «الفرقة N — YEAR» with **exactly two columns — المجموع** (the whole year's total marks, out of that year's total, e.g. `/940`) **+ التقدير** (the year's grade from its GPA). No per-course breakdown for prior years. Trailing (cumulative over all years) = **المجموع التراكمي** (Σ of every year's total marks) · **المعدل التراكمي** (that grand total ÷ number of years, as درجات) · **النسبة المئوية** (grand total ÷ grand max × 100) · **تقدير التخرج** (from CGPA) · **الترتيب** (by cumulative %). Final year = the `academicYear` filter if given, else the latest recorded year; prior years are auto-derived from each graduate's enrollment history.
